  ...
  try {
    ...
    for (const [entryIdStr, coords] of Object.entries(positions)) {
      ...
      if (!isNaN(entryId) && typeof x === 'number' && typeof y === 'number') {
        await db.run(
          'UPDATE codex_entries SET pos_x = ?, pos_y = ? WHERE id = ? AND project_id = ?',
          x,
          y,
          entryId,
          projectId
    ...
    for (const item of positions) {
      if (item.id && typeof item.position === 'number') {
        if (item.parent_id !== undefined) {
          await db.run(
            'UPDATE outline_elements SET position = ?, parent_id = ? WHERE id = ? AND project_id = ?',
            item.position,
            item.parent_id,
            item.id,
            projectId
          ...
          await db.run(
            'UPDATE outline_elements SET position = ? WHERE id = ? AND project_id = ?',
            item.position,
            item.id,
            projectId
          );
...
app.get('/api/settings', async (req, res) => {
  try {
    ...
    for (const row of rows) {
      const val = await AIService.getSetting(row.key);
...
app.post('/api/settings', async (req, res) => {
  ...
  try {
    for (const [key, value] of Object.entries(settingsData)) {
      await AIService.saveSetting(key, String(value));
    }
...
app.get('/api/projects/:projectId/dialogue/analysis', async (req, res) => {
  ...
  try {
    ...
    interface CharacterDialogueStats {
      character: any;
      totalLines: number;
      totalWords: number;
    ...
    const statsMap: Record<number, CharacterDialogueStats> = {};
    ...
    const unassignedQuotes: Array<{ sceneId: number; sceneTitle: string; quote: string }> = [];
    const allWordsByChar: Record<number, Set<string>> = {};
    ...
    for (const scene of scenes) {
      ...
      while ((match = quoteRegex.exec(text)) !== null) {
        ...
        let matchedCharId: number | null = null;
        ...
        for (const char of characters) {
          const names = [char.name, ...(char.aliases || '').split(',').map((a: string) => a.trim())].filter(Boolean);
        ...
        if (matchedCharId && statsMap[matchedCharId]) {
          ...
          words.forEach(w => {
            ...
            if (clean.length > 3) allWordsByChar[matchedCharId!].add(clean);
          });
        } else {
...
3. Output ONLY the rewritten dialogue line inside quotes, followed by a 1-sentence explanation of what vocal changes were made.`;
    ...
    const response = await AIService.generate({ sceneId: 0, prompt: voicePrompt });

...
app.put('/api/projects/:projectId/outline/:id', async (req, res) => {
    ...
    try {
        const db = await (0, database_1.getDatabase)();
        const existing = await db.get('SELECT * FROM outline_elements WHERE id = ? AND project_id = ?', req.params.id, req.params.projectId);
        if (!existing) {
       ...
       WHERE id = ? AND project_id = ?`, updatedParentId, updatedTitle, updatedPosition, updatedSummary, updatedStatus, updatedMetadata, req.params.id, req.params.projectId);
        // Update project timestamp
        await db.run('UPDATE projects SET updated_at = CURRENT_TIMESTAMP WHERE id = ?', req.params.projectId);
...
app.put('/api/scenes/:sceneId/snapshots/:snapshotId/label', async (req, res) => {
    ...
    try {
        const db = await (0, database_1.getDatabase)();
        await db.run('UPDATE scene_snapshots SET label = ? WHERE id = ? AND scene_id = ?', label.trim(), req.params.snapshotId, req.params.sceneId);
        const updated = await db.get('SELECT id, scene_id, word_count, label, source, created_at FROM scene_snapshots WHERE id = ?', req.params.snapshotId);
        if (!updated) {
            return res.status(404).json({ error: 'Snapshot not found' });
        }
        res.json(updated);
    ...
    try {
        ...
        for (const item of events) {
            if (item.id && typeof item.orderIndex === 'number') {
                if (item.track) {
                    await db.run('UPDATE timeline_events SET order_index = ?, track = ? WHERE id = ? AND project_id = ?', item.orderIndex, item.track, item.id, projectId);
                }
                else {
                    await db.run('UPDATE timeline_events SET order_index = ? WHERE id = ? AND project_id = ?', item.orderIndex, item.id, projectId);
                }
            }
        }
...
app.post('/api/projects/:projectId/map/auto-populate', async (req, res) => {
    ...
    `, projectId);
        ...
        for (let i = 0; i < newLocations.length; i++) {
            ...
            if (nameLower.includes('fort') || nameLower.includes('castle') || nameLower.includes('keep'))
                pinType = 'fortress';
            else if (nameLower.includes('forest') || nameLower.includes('river') || nameLower.includes('mountain') || nameLower.includes('sea'))
                pinType = 'wilderness';
            else if (nameLower.includes('gate') || nameLower.includes('portal') || nameLower.includes('realm'))
                pinType = 'portal';
            else if (nameLower.includes('ruin') || nameLower.includes('dungeon') || nameLower.includes('tomb'))
                pinType = 'dungeon';
            await db.run(`
        INSERT INTO map_pins (project_id, codex_location_id, title, x, y, pin_type, notes)
        VALUES (?, ?, ?, ?, ?, ?, ?)
